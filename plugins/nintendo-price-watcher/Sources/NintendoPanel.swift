...
struct NintendoPanelView: View {
    @ObservedObject private var store = NintendoWatcherStore.shared
    @State private var query = ""
    @State private var searching = false
    @State private var results: [NintendoSearchHit] = []
    @State private var pendingHit: NintendoSearchHit?
    @State private var pendingPrice: Double?
    @State private var targetText = ""
    @State private var editGame: NintendoGame?
    ...
    private var header: some View {
        HStack {
            ...
            .help(t("plugin.nintendo.refresh.help"))
    ...
    private var settingsBlock: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text(t("plugin.nintendo.settings.title")).font(.subheadline.weight(.semibold))
            ...
            Text(t("plugin.nintendo.region"))
                .font(.caption)
                .foregroundStyle(.secondary)
            Picker(
                "",
                selection: Binding(
                    get: { store.settings.country },
                    set: { store.setCountry($0) }
                )
            ) {
                ForEach(NintendoCountry.allCases) { cur in
    ...
    private var searchBlock: some View {
        VStack(alignment: .leading, spacing: 8) {
            ...
            if searching {
                HStack(spacing: 8) {
                    ProgressView().controlSize(.small)
                    Text(t("plugin.nintendo.search.loading")).font(.caption).foregroundStyle(.secondary)
                }
            }
            ForEach(results) { hit in
                HStack(spacing: 10) {
                    NintendoCover(imageURL: hit.imageURL)
                    ...
                    VStack(alignment: .leading, spacing: 2) {
                        ...
                        if let price = hit.price {
                            ...
                            Text(PluginL10n.tf("plugin.nintendo.slug", locale: loc, hit.slug))
                                .font(.caption2)
                                .foregroundStyle(.secondary)
                        }
                    }
                    Spacer()
                    let already = store.settings.watchlist.contains(where: { $0.slug == hit.slug })
                    Button(already ? t("plugin.nintendo.in_list") : t("plugin.nintendo.add")) {
    ...
    private var watchlistBlock: some View {
        VStack(alignment: .leading, spacing: 8) {
            Text(PluginL10n.tf("plugin.nintendo.watchlist.title", locale: loc, store.settings.watchlist.count))
                .font(.subheadline.weight(.semibold))
            if store.settings.watchlist.isEmpty {
                Text(t("plugin.nintendo.watchlist.empty"))
                    .foregroundStyle(.secondary)
                    .font(.callout)
            } else {
    ...
    private var sortedGames: [NintendoGame] {
    ...
    private func gameRow(_ game: NintendoGame) -> some View {
        VStack(alignment: .leading, spacing: 8) {
            HStack(alignment: .top, spacing: 10) {
                NintendoCover(imageURL: game.imageURL)
                    .frame(width: 92, height: 34)
                    .clipped()
                VStack(alignment: .leading, spacing: 2) {
                    HStack(spacing: 6) {
                    ...
                    Text(PluginL10n.tf("plugin.nintendo.target", locale: loc, store.settings.currencySymbol, game.targetPrice))
                        .font(.caption)
                        .foregroundStyle(.secondary)
                }
                Spacer()
                VStack(alignment: .trailing, spacing: 2) {
                    if let price = game.currentPrice {
                        ...
                        if game.isOnTarget {
                            ...
                            Text(PluginL10n.tf("plugin.nintendo.below_target", locale: loc, saved))
                                .font(.caption2)
                                .foregroundStyle(.secondary)
                        }
                    } else {
            ...
            HStack {
                Button(t("plugin.nintendo.edit_target")) {
                    editTarget = String(format: "%.2f", game.targetPrice)
                    editGame = game
                }
                .controlSize(.small)
                Button(t("plugin.nintendo.open_dekudeals")) {
                    NSWorkspace.shared.open(DekuDealsAPI.itemURL(slug: game.slug))
                }
                .controlSize(.small)
                Spacer()
                Button(role: .destructive) {
                    store.removeGame(slug: game.slug)
                } label: {
    ...
    private func addSheet(_ hit: NintendoSearchHit) -> some View {
        VStack(alignment: .leading, spacing: 14) {
            Text(t("plugin.nintendo.add_sheet.title")).font(.title3.weight(.semibold))
            HStack(spacing: 10) {
                NintendoCover(imageURL: hit.imageURL)
                    .frame(width: 92, height: 34)
                    .clipped()
            ...
            if let pendingPrice {
                Text(PluginL10n.tf("plugin.nintendo.add_sheet.current_price", locale: loc, store.settings.currencySymbol, pendingPrice))
                    .foregroundStyle(.secondary)
            } else {
                Text(t("plugin.nintendo.add_sheet.no_price"))
                    .foregroundStyle(.secondary)
            }
            TextField(PluginL10n.tf("plugin.nintendo.add_sheet.target_field", locale: loc, store.settings.currencySymbol), text: $targetText)
                .textFieldStyle(.roundedBorder)
            HStack {
                Spacer()
                Button(PluginL10n.t("common.cancel", locale: loc)) { pendingHit = nil }
                Button(t("plugin.nintendo.add")) {
    ...
    private func editSheet(_ game: NintendoGame) -> some View {
        VStack(alignment: .leading, spacing: 14) {
            Text(t("plugin.nintendo.edit_sheet.title")).font(.title3.weight(.semibold))
            Text(game.name)
            TextField(PluginL10n.tf("plugin.nintendo.add_sheet.target_field", locale: loc, store.settings.currencySymbol), text: $editTarget)
                .textFieldStyle(.roundedBorder)
            HStack {
                Spacer()
                ...
                Button(PluginL10n.t("plugin.common.save", locale: loc)) {
                    if let v = Double(editTarget.replacingOccurrences(of: ",", with: ".")) {
                        store.updateTarget(slug: game.slug, targetPrice: v)
                    }
                    editGame = nil
                }
                .buttonStyle(.borderedProminent)
    ...
    private func runSearch() async {
        ...
        do {
            ...
            results = []
            store.setError(friendlyNetworkError(error))
        }
    }
    ...
    private func friendlyNetworkError(_ error: Error) -> String {
        ...
        if ns.domain == NSURLErrorDomain, ns.code == NSURLErrorBadServerResponse {
            return "Deku Deals blocked the request (Cloudflare). Try again in a few seconds."
        }
        return error.localizedDescription
    }

    private func prepareAdd(_ hit: NintendoSearchHit) async {
        var price = hit.price
        if price == nil {
            price = try? await DekuDealsAPI.fetchPrice(slug: hit.slug, country: store.settings.country).price
        }
        pendingPrice = price
...
private struct NintendoCover: View {
    let imageURL: String?
