...
public final class StatsUptimePlugin: AlwmPlugin {
    ...
    public func barItem(placement: AlwmBarPlacement) -> NSView? {
        ...
        return UptimeBarChipView(
            label: UptimeStore.shared.barLabel,
            scale: scale,
            tooltip: UptimeStore.shared.tooltip
        )
    }
...
private final class UptimeBarChipView: NSView {
    ...
    override func mouseDown(with event: NSEvent) {
        Task { @MainActor in
            UptimePanelController.toggle(relativeTo: self)
        }
    }
